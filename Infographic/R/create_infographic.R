...
outdir <- ("WubbleYouOutput/")
new_file <- read.csv("merged_IDs.csv") # Finished participants

### Retrieve user ID to test individuals
# ID <- new_file$User.ID[1] # Take matching ID
# ID

### PDF Script ###
key <- "key" # Requires Google Maps API account.
...
create_infographic <-  function(d1,d2,ID,outdir){
  ...
  stay <- d2_s[c("Staypoint.Name","Staypoint.Hardcoded.Location")]
  stay <- stay[stay!= ""]
  n.quest <- length(stay)
  ...
  act <- d2_s[c("Staypoint.Activity","Staypoint.Second.Activity")]
  act <- act[act!=""]
  ...
  #Generate 'with' list
  with <- d2_s$Staypoint.People   #Change to person with
  with <- with[with!=""]
  ...
  iH <- which(d2_s$Staypoint.Name == txtHome | d2_s$Staypoint.Hardcoded.Location == txtHome)
