...
As part of our mobile survey we want to report back to our participants --  that is to share the data we have collected about them with them, after they have finished their participation. We needed to identify people from our much larger dataset using their Participant ID, and then produce a summary of their participation via the data.

To do so we wanted to create an anonymous and automated piece of R analysis that produced a production quality graphic that summarizes their data. To our surprise there were few examples of this type of output on R, despite the fact that combining multiple plots, graphics, and text on a single figure is well documented in the R infrastructure. We did however find one nice [example](https://www.r-bloggers.com/r-how-to-layout-and-design-an-infographic/) and we chose to build on the ideas that were used in that blogpost.


## Infographic Components

Our survey consisted of two separate but related datasets:
...
1. Summary statistics about the participant and their data;
2. Maps of participant GPS tracking data, overlaid on a basemap/satellite imagery;
3. Frequency histograms of responses from three different survey questions about feelings;
...
- one centered on their most popular check-in location, often home;
- one centered on their second most popular check-in location, often work.
